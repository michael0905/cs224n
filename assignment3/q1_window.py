...
class Config:
    ...
    ### YOUR CODE HERE
    n_window_features = 2 * window_size * n_word_features + n_word_features # The total number of features used for each window.
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        #     features = []
        #     for feature in new_sentence[i : 2 * window_size + i + 1]:
        #         features += feature
        #     label = labels[i]
        #     windowed_data.append((features, label))
        orig_n = len(sentence)
        # extend sentence
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, [-1, self.config.n_window_features * self.pretrained_embeddings.shape[1]])
