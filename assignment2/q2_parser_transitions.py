...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies = []
    num = len(partial_parses) // batch_size
    for i in range(num):
        mini_batch = partial_parses[i * batch_size : (i+1) * batch_size]
        while len(mini_batch) > 0:
            minibatch_transitions = model.predict(mini_batch)
            for i,transition in enumerate(minibatch_transitions):
                mini_batch[i].parse_step(transition)
            mini_batch = [parse for parse in mini_batch if len(parse.buffer) > 0 or len(parse.stack) > 1]
    # last part
    mini_batch = partial_parses[num * batch_size :]
    while len(mini_batch) > 0:
        minibatch_transitions = model.predict(mini_batch)
        for i,transition in enumerate(minibatch_transitions):
            mini_batch[i].parse_step(transition)
