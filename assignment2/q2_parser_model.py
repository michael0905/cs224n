...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings = tf.nn.embedding_lookup(params=self.pretrained_embeddings, ids=self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.Variable(tf.random_uniform(shape=[self.config.hidden_size,]))
        b2 = tf.Variable(tf.random_uniform(shape=[self.config.n_classes]))
    ...
    def run_epoch(self, sess, parser, train_examples, dev_set):
        ...
        print "- dev UAS: {:.3f}".format(dev_UAS * 100.0)
        return dev_UAS
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            if not debug:
                ...
                print "- test UAS: {:.3f}".format(UAS * 100.0)
